...
const ctx = (dossier: Dossier, artifact: Artifact, deps: CheckDeps = {}) => ({ dossier, artifact, deps })

describe('CLAIM_COVERAGE', () => {
  it('passes a supported sentence with a matching number', async () => {
    const art = mkArtifact({ sentences: [{ text: 'Reduced latency by 38%.', claimIds: ['CLM-1'] }] })
...
describe('EVIDENCE_RESOLVES', () => {
  ...
  it('fails an unreadable, missing-file document (broken-asset law)', async () => {
    const d = mkDossier({
      evidence: [EvidenceItemSchema.parse({ id: 'EV-2', kind: 'document', label: 'cert', sourceRef: 'missing.pdf', contentText: '' })],
      claims: [],
    })
...
describe('LINK_LIVENESS', () => {
  ...
  it('passes when links resolve live (mocked fetcher)', async () => {
    const art = mkArtifact({ sentences: [{ text: 'Portfolio at https://good.example/me', claimIds: ['CLM-1'] }] })
    expect((await LINK_LIVENESS.run(ctx(mkDossier(), art, deps))).status).toBe('pass')
  })
  it('fails on a dead link', async () => {
    const art = mkArtifact({ sentences: [{ text: 'See https://dead.example/404', claimIds: ['CLM-1'] }] })
...
describe('PLACEHOLDER_TEXT', () => {
  it('passes clean prose', async () => {
    const art = mkArtifact({ sentences: [{ text: 'Led the migration to TypeScript.', claimIds: ['CLM-1'] }] })
    expect((await PLACEHOLDER_TEXT.run(ctx(mkDossier(), art))).status).toBe('pass')
  })
  it('fails on [BRACKETS] and TBD', async () => {
    const art = mkArtifact({ sentences: [{ text: 'Worked at [COMPANY NAME] on TBD projects.', claimIds: ['CLM-1'] }] })
...
describe('DATE_SANITY', () => {
  ...
  it('fails a future date (2099)', async () => {
    const d = mkDossier({ profile: { fullName: 'X', timezone: 'America/New_York', experiences: [{ org: 'Y', title: 'Z', startYm: '2099-01', endYm: null }] } })
  ...
  it('passes when a claim renders the same number everywhere', async () => {
    const d = mkDossier({
      artifacts: [
        ...
        { id: 'B', kind: 'cover_letter', sentences: [{ text: 'again grew 38%', claimIds: ['CLM-1'] }] },
      ],
    })
...
describe('FORMAT_LAW', () => {
  it('passes a single-column ATS resume with approved headings', async () => {
    const art = mkArtifact({ meta: { html: '<h2>EXPERIENCE</h2><p>Did things.</p><h2>SKILLS</h2>' } })
    expect((await FORMAT_LAW.run(ctx(mkDossier(), art))).status).toBe('pass')
  })
  it('fails a two-column (table) ATS layout', async () => {
    const art = mkArtifact({ meta: { html: '<table><tr><td>left</td><td>right</td></tr></table>' } })
...
describe('DOCX_INTEGRITY', () => {
  it('passes when the docx reopens with headings', async () => {
    ...
    const deps = { readDocx: async () => ({ text: 'EXPERIENCE ... SKILLS', headings: ['EXPERIENCE', 'SKILLS'] }) }
...
describe('ATS_PARSE_BACK', () => {
  ...
  it('fails when the engine reports field diffs', async () => {
    const deps = { parseBack: async () => ({ fidelityPct: 80, fieldDiffs: [{ field: 'title', expected: 'SWE', got: 'SVVE' }] }) }
...
describe('CONTACT_VALIDITY', () => {
  ...
  it('fails a malformed link', async () => {
    const d = mkDossier({ profile: { fullName: 'X', timezone: 'UTC', contact: { links: ['not a url'] } } })
...
describe('PII_HYGIENE', () => {
  it('passes when only approved fields are exposed', async () => {
    const art = mkArtifact({ meta: { shareView: true, approvedFields: ['email'], html: 'Reach me at c@example.com' } })
  ...
  it('fails when an unapproved phone number leaks into a share view', async () => {
    const d = mkDossier({ profile: { fullName: 'X', timezone: 'UTC', contact: { phone: '+15550100', links: [] } } })
    const art = mkArtifact({ meta: { shareView: true, approvedFields: [], html: 'call +15550100' } })
...
describe('JD_COVERAGE', () => {
  it('is informational and never fails', async () => {
    const d = mkDossier({ brief: { jdText: 'Need Rust', decomposed: [{ id: 'R1', text: 'Rust', kind: 'must', keywords: ['rust'] }] } })
