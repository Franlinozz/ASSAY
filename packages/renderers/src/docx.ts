...
export async function buildResumeDocx(dossier: Dossier, bullets: Sentence[]): Promise<Uint8Array> {
  ...
  children.push(new Paragraph({ children: [new TextRun({ text: p.fullName, bold: true, size: 32 })] }))
  if (p.headline) children.push(new Paragraph({ children: [new TextRun({ text: p.headline, size: 22 })] }))
  ...
  if (p.experiences.length > 0) {
    ...
    p.experiences.forEach((exp, i) => {
      children.push(new Paragraph({ children: [new TextRun({ text: `${exp.org} — ${exp.title}`, bold: true })] }))
      children.push(new Paragraph({ children: [new TextRun({ text: `${exp.startYm} – ${exp.endYm ?? 'Present'}`, italics: true, size: 18 })] }))
      for (const b of bulletsForExperience(dossier, i, bullets)) children.push(new Paragraph({ text: b.text, bullet: { level: 0 } }))
    })
  }
